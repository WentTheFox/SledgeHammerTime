...
class ProfileController extends Controller {
  ...
  public function edit(Request $request):Response {
    ...
    if ($authUser){
      ...
      $crowdinUsers = self::getCrowdinUsersForAuthUser($authUser);
    }
   ...
   * @return list<mixed>
   */
  static function getCrowdinUsersForAuthUser(User $authUser):array {
    $crowdinProjectId = config('services.crowdin.project_id');

    return $authUser->crowdinUsers()->with(['translators', 'creditOverrides', 'creditOverrideProposals'])->get()->map(function (CrowdinUser $cu) use ($crowdinProjectId) {
