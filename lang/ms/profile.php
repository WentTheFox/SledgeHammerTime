<?php

return [
  'title' => 'Profil',
  'intro' => 'Halaman profil membolehkan anda menguruskan maklumat akaun anda khususnya yang berkaitan dengan laman sesawang. Sekiranya anda mencari cara untuk mengubah nilai lalai dan tetapan lain untuk aplikasi Discord, lawati halaman <1>Tetapan Apl</1>.',
  'information' => [
    'heading' => 'Maklumat Profil',
    'description' => "Kemas kini maklumat profil akaun anda.",
    'displayName' => 'Nama Paparan',
    'hiddenFormats' => 'Format yang disembunyikan dari jadual pemungut',
    'saveSuccess' => 'Profil anda telah berjaya dikemas kini.',
  ],
  'accounts' => [
    'heading' => 'Akaun Tersambung',
    'description' => 'Anda boleh memautkan beberapa akaun anda ke akaun HammerTime anda untuk membolehkan pengurusan di satu tempat. Maklumat asas pengguna yang dipaparkan di sini dikemaskinikan secara automatik maksimum sejam sekali apabila anda memuatkan halaman ini.',
    'discord' => 'Akaun Discord',
    'crowdin' => 'Akaun Crowdin',
    'noConnectedAccounts' => 'Anda tidak mempunyai sebarang akaun yang tersambung ketika ini',
    'linkAdditional' => [
      'heading' => 'Pautkan Akaun Tambahan',
      'description' => 'Gunakan butang di bawah untuk menyambungkan akaun tembahan ke akaun HammerTime anda yang semasa.',
      'discord' => [
        'description' => 'Sekiranya gambar profil atau nama paparan anda berubah, anda juga boleh menggunakan butang ini untuk mengemaskinikan maklumat tersebut bagi sebarang akaun Discord yang sudah dipautkan.',
        'authorize' => 'Beri keizinan akaun Discord',
      ],
      'crowdin' => [
        'description' => 'Sekiranya anda seorang penterjemah bagi projek ini, pemautan akaun Crowdin anda membolehkan anda mengubah suai cara nama anda dipaparkan pada penghargaan terjemahan, dan juga memberikan tahap pengesahan tambahan bahawa akaun ini dimiliki oleh anda sekiranya anda menyertai pelayan Discord projek.',
        'authorize' => 'Beri keizinan akaun Crowdin',
      ],
    ],
  ],
  'creditOverrides' => [
    'heading' => 'Penghargaan Terjemahan',
    'description' => 'Ubah suai cara diri anda dipaparkan pada penghargaan terjemahan bagi setiap bahasa yang mana anda seorang penyumbang.',
    'updateMethodLabel' => 'Kemas kini kaedah',
    'updateMethodDescription' => 'Menyalin daripada bahasa lain tidak memerlukan semakan manual',
    'updateMethodCopyLabel' => 'Salin daripada bahasa lain',
    'updateMethodEditLabel' => 'Nyatakan nilai individu',
    'creditSourceLanguage' => 'Bahasa untuk disalin',
    'displayName' => 'Nama Paparan',
    'avatarProvider' => 'Avatar',
    'avatarProviders' => [
        'discord' => 'Discord',
        'gravatar' => 'Gravatar',
        'crowdin' => 'Crowdin',
    ],
    'gravatarEmail' => 'Alamat e-mel',
    'gravatarDescription' => '<1>Gravatar</1> membolehkan anda menggunakan satu avatar merentasi pelbagai laman sesawang yang menyokongnya. Alamat e-mel anda akan disimpan sebagai cincangan MD5.',
    'avatarAccount' => 'Akaun',
    'url' => 'Pautan',
    'visibility' => 'Keterlihatan',
    'submit' => 'Hantar untuk kelulusan',
    'saveSuccess' => 'Dihantar untuk kelulusan',
    'saveHideSuccess' => 'Keterlihatan dikemas kini',
    'copySuccess' => 'Penghargaan disalin',
    'cancel' => 'Batalkan perubahan',
    'statusPrefix' => 'Status',
    'status' => [
      'approved' => 'Diluluskan',
      'inReview' => 'Dalam semakan',
      'rejected' => 'Ditolak',
    ],
    'currentlyApproved' => 'Nilai yang diluluskan ketika ini',
    'reApprovalWarning' => 'Mengubah apa-apa selain keterlihatan akan memerlukan semakan manual. Anda boleh batalkan permintaan semakan pada bila-bila masa dari halaman ini.',
    'deleteOverride' => 'Alih keluar pengatasan',
    'deleteOverrideConfirm' => 'Adakah anda pasti anda ingin mengalih keluar pengatasan penghargaan terjemahan anda? Entri anda akan dikembalikan kepada maklumat lalai daripada Crowdin.',
    'approvedValues' => [
      'displayName' => 'Nama',
      'avatarProvider' => 'Penyedia avatar',
      'avatarId' => 'ID avatar',
      'url' => 'URL profil',
      'visible' => 'Kelihatan',
      'hidden' => 'Disembunyikan',
    ],
  ],
  'deletion' => [
    'heading' => 'Pemadaman Akaun',
    'description' => 'Apabila akaun anda dipadamkan, kesemua sumber dan datanya akan dipadamkan secara kekal. Sebelum memadam akaun anda, sila muat turun sebarang data atau maklumat yang anda ingin simpan.',
    'deleteButton' => 'Padam Akaun',
    'confirmDialog' => [
      'header' => 'Adakah anda pasti anda ingin memadamkan akaun anda?',
      'body' => 'Apabila akaun anda dipadam, kesemua sumber dan datanya akan dipadamkan secara kekal.',
    ],
  ],
];
